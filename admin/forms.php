...
<div class="admin-header-flex" style="margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center;">
    <div>
        ...
        <select id="form-selector" onchange="loadForm(this.value)" style="margin-top:0.5rem; padding:0.4rem; border-radius:4px;">
            <?php foreach ($forms as $index => $form): ?>
                <option value="<?= $index ?>"><?= htmlspecialchars($form['title']) ?> (<?= htmlspecialchars($form['id']) ?>)</option>
            <?php endforeach; ?>
        </select>
        <button class="btn" style="background:#e2e8f0; color:#333; padding: 0.4rem 0.8rem; font-size: 0.9rem;" onclick="createNewForm()">+ Nieuw</button>
    </div>
...
function createNewForm() {
    const id = prompt("Geef het formulier een ID (bijv. 'nieuw-form'):");
    if (!id) return;
    ...
    const newForm = {
        ...
        admin_email: "letsgo@grutdesigners.nl",
    ...
    opt.value = activeFormIndex;
    opt.textContent = `${newForm.title} (${newForm.id})`;
    sel.appendChild(opt);
    sel.value = activeFormIndex;
