...
try {
    ...
    $stmt = $pdo->query("SELECT id, slug, title FROM forms ORDER BY title ASC");
    $nav_forms = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    // Silent fail
}
?>
    <div class="nav-group <?= $is_forms_active ? 'is-active' : '' ?>">
        <a href="forms.php" class="nav-item nav-item-parent <?= ($current_page == 'forms.php' && !isset($_GET['id']) && !isset($_GET['new'])) ? 'active' : '' ?>">
            Formulieren
            <span class="nav-arrow">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>
            </span>
        ...
        <div class="nav-submenu">
            <?php foreach ($nav_forms as $form): ?>
                <a href="form_editor.php?id=<?= $form['id'] ?>" class="nav-item nav-subitem <?= ($current_page == 'form_editor.php' && isset($_GET['id']) && $_GET['id'] == $form['id']) ? 'active' : '' ?>">
