...
- [x] **Story 2: Admin Media Hub** - Grid weergave, verwijderen, pad-kopieer-acties en tag-beheer.
...
**Taken / User Stories**
- [x] **Story 1: User Rollen & Authenticatie** - SQLite tabel `users` uitbreiden/aanmaken met rollen: `SuperAdmin` (kan alles, inclusief developer tools) en `Redacteur` (kan alleen CMS en Media benaderen).
- [x] **Story 2: UI Permissies** - Verberg irrelevante menu-items voor redacteuren (zoals Database resets of Code-weergaven).
- [x] **Story 3: Git Sync Workflow** - (Optioneel) Een 'Deploy' knop voor SuperAdmins waarmee eventueel gewijzigde file-storage structuur (`storage/`) via een background worker op de live server veilig gesynct kan worden. 
